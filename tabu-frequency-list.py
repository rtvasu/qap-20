...
class TabuSearch:
    ...
    def evaluateCandidates(self, cost):
        ...
        for c in candidates:
            ...
            i,j = c
            self.swap(p, i, j)
            # add frequency penalty in picking candidate
            current_cost = self.findCost(p) + frequency[i][j]
            if (current_cost < best_cost):
                best_cost = current_cost
                best_candidate = (i, j)
        # aspiration criteria: if a tabu move is the best we've seen, we override the other candidates and pick it
        for t in tabus:
            p = copy.deepcopy(permutation)
            i,j = t
            self.swap(p, i, j)
...
frequency = np.zeros((num_locations, num_locations), dtype = int)
