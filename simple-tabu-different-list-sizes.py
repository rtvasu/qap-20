...
tabus = []
tabu_list_sizes = [2, 4, 8, 17]
print()
for k in range(len(tabu_list_sizes)):
    permutation = [i for i in range(num_locations)]
    size = tabu_list_sizes[k]
    ...
    class TabuSearch:
        ...
        def tabuSearch(self):
            ...
            print('initial permutation: %s, cost %s, size: %s ' % (permutation, cost, size))
    ...
    print('best solution found: %s, cost %s' % (search[0], search[1]))
    print()